...
def update_views_count() -> None:
    default_logger.debug("Run update-views task")
    asyncio.run(update_views_count_async_task())

async def update_views_count_async_task() -> None:
    
    default_logger.info("Celery task. Update views count. Start")
    ...
    # так как задача запускается в новом event loop
    ...
    try:
        ...
        default_logger.debug("Celery task. Update views count. executing query")
        ...
        default_logger.debug("Celery task. Update views count. executing query done")
        ...
        default_logger.info("Updating views count: Done")
